 ...
 * used for argument/return later.  The syntax of enum is same as C but
 * it only accepts a simple integer contant or other enum constant in RHS.
 *
...
int parse_enum_string(char *enum_str)
{
	...
	while ((pos = strsep(&tmp, ";")) != NULL) {
		...
		while (ret != TOKEN_NULL && strcmp(enum_token, "}")) {
			...
			/* TODO: sort by value, just in case */
			list_add(&e_val->list, &e_def->vals);
...
TEST_CASE(argspec_parse_enum)
{
	...
	str = get_enum_string("uftrace", 24);
